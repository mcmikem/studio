    ...
    const smartRemindersPrompt = ai.definePrompt(
      {
        ...
        **Here are good examples of the tone and specificity required:**
        ...
        prompt: `Now, generate a short list of 3-4 smart, actionable reminders for ${input.userName} (Role: ${input.userRole}). Use the getUpcomingEventsForUser and getPendingTasksForUser tools with userId '${input.userId}' to get the necessary data.`,
      }
    );
