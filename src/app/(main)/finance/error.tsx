  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center bg-white p-6 text-center rounded-2xl shadow-comic-sm border my-4">
      <div className="mb-8 relative">
         ...
         <OmutoLoader size="lg" label="Financial Interruption" />
      </div>
      ...
      <div className="max-w-lg space-y-6 relative z-10 w-full">
        <div className="inline-flex items-center gap-2 px-4 py-2 bg-rose-500/10 rounded-full text-rose-600 mb-2">
            ...
            <span className="text-[9px] font-black uppercase tracking-widest">Finance Module Anomaly</span>
        </div>
        
        <h1 className="text-3xl sm:text-4xl font-black text-omuto-navy uppercase tracking-tight">Financial Divergence</h1>
        <p className="text-[10px] sm:text-xs font-bold text-muted-foreground uppercase tracking-widest leading-relaxed">
          The Finance module has encountered a protocol discrepancy. 
          Accounting stability is being restored. You can safely return to the dashboard.
        </p>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-8">
            <Button onClick={() => reset()} className="btn-omuto h-12 rounded-2xl px-8 font-black uppercase tracking-widest text-[9px] w-full sm:w-auto shadow-comic bg-omuto-navy text-white hover:bg-omuto-navy/90">
                <RefreshCw className="mr-2 h-4 w-4" /> Reload Finance
            </Button>
            <Button variant="outline" className="h-12 rounded-2xl px-6 border-2 font-black uppercase tracking-widest text-[9px] w-full sm:w-auto text-omuto-navy bg-white" onClick={() => window.open('mailto:support@omuto.org?subject=Finance Error&body=' + encodeURIComponent(error.message || 'Finance error occurred'))}>
