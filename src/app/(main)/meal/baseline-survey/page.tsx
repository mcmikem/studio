...
const baselineSchema = z.object({
  ...
  primaryChallenge: z.string().min(10, 'Please describe their primary challenge.'),
  programGoals: z.string().min(10, 'Please describe their goals.'),
});

type BaselineFormData = z.infer<typeof baselineSchema>;

function BaselineSurveyForm() {
  ...
  const onSubmit = async (data: BaselineFormData) => {
    ...
    try {
      await addDocumentNonBlocking(collection(firestore, 'baseline-surveys'), record);
      toast({
        title: 'Baseline Survey Saved!',
        description: `The "before" data has been logged.`,
      });
      reset();
    } catch (error: any) {
       ...
       <Button variant="outline" asChild>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Capture the "before the program" status for a beneficiary.
            </CardDescription>
          </CardHeader>
          <form onSubmit={handleSubmit(onSubmit)}>
            <CardContent className="space-y-6">
               ...
               <div className="space-y-2">
                ...
                <Input id="monthlyIncome" type="number" {...register('monthlyIncome')} placeholder="e.g., 50000" />
              </div>

               <div className="space-y-2">
                <Label htmlFor="primaryChallenge">Primary Challenge</Label>
                <Textarea id="primaryChallenge" {...register('primaryChallenge')} placeholder="Describe the main challenge this person is facing." />
                {errors.primaryChallenge && <p className="text-sm text-destructive">{errors.primaryChallenge.message}</p>}
              </div>
              <div className="space-y-2">
                <Label htmlFor="programGoals">Goals for Program</Label>
                <Textarea id="programGoals" {...register('programGoals')} placeholder="What does this person hope to achieve?" />
                {errors.programGoals && <p className="text-sm text-destructive">{errors.programGoals.message}</p>}
              </div>
