      ...
      <CardContent className="space-y-4">
        ...
        {checkouts && checkouts.length > 0 ? (
          checkouts.map((checkout) => (
            <div key={checkout.id} className="flex flex-col border-b pb-3 last:border-b-0">
              <p className="text-sm text-muted-foreground">{checkout.task}</p>
              <p className="text-xs text-muted-foreground mt-1">
                {checkout.timestamp ? formatDistanceToNow(checkout.timestamp.toDate(), { addSuffix: true }) : ''}
              </p>
            </div>
          ))
        ) : (
          !isLoading && <p className="text-sm text-muted-foreground">No checkout reports found.</p>
        )}
      </CardContent>
    </Card>
  );
    ...
    <div className="flex flex-col gap-6">
      <header>
        ...
        <p className="text-muted-foreground">
          Your personal information and recent activity.
        </p>
      </header>
